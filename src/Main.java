...
public class Main {
    public static void main(String[] args) {
        ...
        Car kia = new Car("Kia", "Sportage 4-го поколения", 2.4,
                "Red", 2018, "Южная Корея");
        Car hyundai = new Car("Hyundai", "Avante", 1.6
                , "Orange", 2016, "Южная Корея");
        printInfo(lada);
        printInfo(audi);
        printInfo(bmw);
        printInfo(kia);
        ...
        private static void printInfo(Car car){
            System.out.println("Car{" +
                    ...
                    ", typeOfRubber=" + car.isTypeOfRubber() +
                    ", key=" + car.getKey().isKeyAccess()  +
                    ", key=" + car.getKey().isRemoteRunEngine()  +
                    ", insurance=" + car.getInsurance().getNumber() +
                    '}');
